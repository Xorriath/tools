...
def iface_finder():
    ...
    if system_platform == 'Linux':
        ...
        for line in ipr.communicate()[0].decode('utf-8').splitlines():
            if 'default' in line:
                l = line.split()
                iface = l[4]
                return iface
    elif system_platform == 'Darwin':  # OSX support
        return check_output("route get 0.0.0.0 2>/dev/null| sed -n '5p' | cut -f4 -d' '", shell=True).decode('utf-8').rstrip()
    else:
...
def pkt_parser(pkt):
    ...
    if pkt.haslayer(UDP) and pkt.haslayer(IP):
        ...
        # Kerberos over UDP
        decoded = Decode_Ip_Packet(bytes(pkt)[14:])
    ...
    # TCP
        ...
        full_load_bytes = pkt_frag_loads[src_ip_port][ack]

        # Decode to string for text protocol parsing
        try:
            full_load = full_load_bytes.decode('utf-8', errors='replace')
        except (UnicodeDecodeError, AttributeError):
            full_load = str(full_load_bytes)
        ...
        other_parser(src_ip_port, dst_ip_port, full_load, full_load_bytes, ack, seq, pkt, parse_args().verbose)
    ...
    if MsgType == b"\x0a" and EncType == b"\x17" and MessageType == b"\x02":
        if Data[49:53] == b"\xa2\x36\x04\x34" or Data[49:53] == b"\xa2\x35\x04\x33":
            ...
            if HashLen == 54:
                ...
                BuildHash = "$krb5pa$23$"+Name+"$"+Domain+"$dummy$"+SwitchHash.hex()
        ...
        if Data[44:48] == b"\xa2\x36\x04\x34" or Data[44:48] == b"\xa2\x35\x04\x33":
            ...
            Name = Data[HashLen+97:HashLen+97+NameLen].decode('latin-1')
            DomainLen = struct.unpack('<b',Data[HashLen+97+NameLen+3:HashLen+97+NameLen+4])[0]
            Domain = Data[HashLen+97+NameLen+4:HashLen+97+NameLen+4+DomainLen].decode('latin-1')
            BuildHash = "$krb5pa$23$"+Name+"$"+Domain+"$dummy$"+SwitchHash.hex()
            return 'MS Kerberos: %s' % BuildHash
    ...
    if MsgType == b"\x0a" and EncType == b"\x17":
        try:
            if Data[40:44] == b"\xa2\x36\x04\x34" or Data[40:44] == b"\xa2\x35\x04\x33":
                ...
                Name = Data[HashLen+98:HashLen+98+NameLen].decode('latin-1')
                DomainLen = struct.unpack('<b',Data[HashLen+98+NameLen+3:HashLen+98+NameLen+4])[0]
                Domain = Data[HashLen+98+NameLen+4:HashLen+98+NameLen+4+DomainLen].decode('latin-1')
                BuildHash = "$krb5pa$23$"+Name+"$"+Domain+"$dummy$"+SwitchHash.hex()
                return 'MS Kerberos: %s' % BuildHash
        except struct.error:
            return
...
def Decode_Ip_Packet(s):
    ...
    '''
    d={}
    # In Python 3, indexing bytes returns int, no ord() needed
...
def other_parser(src_ip_port, dst_ip_port, full_load, full_load_bytes, ack, seq, pkt, verbose):
    '''
    Pull out pertinent info from the parsed HTTP packet data
    full_load: decoded string for text protocol parsing
    full_load_bytes: raw bytes for binary protocol parsing (NTLM)
    '''
    user_passwd = None
    http_url_req = None
    method = None
    ...
    if body != '':
        ...
        if user_passwd != None:
            # user_passwd is already strings from regex match
    ...
    # Use bytes for binary protocol matching
    NTLMSSP2 = re.search(NTLMSSP2_re, full_load_bytes, re.DOTALL)
    NTLMSSP3 = re.search(NTLMSSP3_re, full_load_bytes, re.DOTALL)
    if NTLMSSP2:
...
def parse_basic_auth(src_ip_port, dst_ip_port, headers, authorization_header):
    ...
    if authorization_header:
        ...
        if b64_auth_re != None:
            ...
            try:
                basic_auth_creds = base64.decodebytes(basic_auth_b64.encode('ascii')).decode('utf-8', errors='replace')
            except Exception:
                return
...
def parse_snmp(src_ip_port, dst_ip_port, snmp_layer):
    ...
    '''
    community_val = snmp_layer.community.val
    if isinstance(community_val, bytes):
        community_val = community_val.decode('utf-8', errors='replace')
    if isinstance(community_val, str):
        ver = snmp_layer.version.val
        msg = 'SNMPv%d community string: %s' % (ver, community_val)
        printer(src_ip_port, dst_ip_port, msg)
    return True
...
def parse_netntlm_chal(headers, chal_header, ack):
    ...
    if header_val2[0] == 'NTLM' or header_val2[0].lower() == 'negotiate':
        try:
            ...
            return
        msg2 = base64.decodebytes(msg2.encode('ascii'))
...
def parse_netntlm_resp_msg(headers, resp_header, seq):
    ...
    if header_val3[0] == 'NTLM' or header_val3[0] == 'Negotiate':
        try:
            msg3 = base64.decodebytes(header_val3[1].encode('ascii'))
        except binascii.Error:
            return
...
def parse_ntlm_resp(msg3, seq):
    ...
    if len(msg3) > 43:
        ...
        lmhash = binascii.b2a_hex(msg3[lmoff:lmoff+lmlen]).decode('ascii')
        nthash = binascii.b2a_hex(msg3[ntoff:ntoff+ntlen]).decode('ascii')
        domain = msg3[domoff:domoff+domlen].replace(b"\0", b"").decode('utf-16-le', errors='ignore')
        user = msg3[useroff:useroff+userlen].replace(b"\0", b"").decode('utf-16-le', errors='ignore')
